...
export function feedHead(ort, hidden, b) {
    ...
    const H = new ort.Tensor("float32", Float32Array.from(hidden.flat(Infinity).map(Number)), [hidden.length ?? n, hidden[0]?.length ?? 1, hidden[0]?.[0]?.length ?? 1]);
    ...
    return {
        ...
        marker_mask: new ort.Tensor("bool", Uint8Array.from(b.markerMask.flat(Infinity).map((v) => (v ? 1 : 0))), [n, k]),
